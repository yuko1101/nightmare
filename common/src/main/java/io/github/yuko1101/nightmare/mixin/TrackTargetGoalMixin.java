...
import io.github.yuko1101.nightmare.entity.goals.IPathCreateGoal;
...
public class TrackTargetGoalMixin {
    ...
    private boolean modifyVisibilityChecks(boolean checkVisibility) {
        return checkVisibility && ((MobEntityAccessor) this.mob).getGoalSelector().getGoals().stream().noneMatch((goal) -> goal.getGoal() instanceof IPathCreateGoal);
    }
    ...
    private boolean modifyNavigationChecks(boolean checkCanNavigate) {
        return checkCanNavigate && ((MobEntityAccessor) this.mob).getGoalSelector().getGoals().stream().noneMatch((goal) -> goal.getGoal() instanceof IPathCreateGoal);
    }
}
